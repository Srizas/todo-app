...
export default function App() {
  ...
  // États des cours et des tâches (initialisés vides, le chargement se fait via useEffect)
  ...
  // 1. Charger les données au démarrage du composant
  useEffect(() => {
    const now = new Date();
    setTodayDate(now);
    ...
    if (savedCourses) {
      ...
      // Données de démo par défaut si le stockage est totalement vide
      setCourses([
    ...
    setIsLoaded(true); // Verrouille l'état d'initialisation pour éviter d'écraser direct
  }, []);

  // 2. Sauvegarder les cours à chaque changement (uniquement après chargement initial)
  useEffect(() => {
    if (isLoaded) {
  ...
  // 3. Sauvegarder les To-Dos à chaque changement (uniquement après chargement initial)
  useEffect(() => {
    if (isLoaded) {
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex space-x-1 bg-slate-900 p-1 rounded-xl border border-slate-700/50">
            ...
            <button onClick={() => setActiveTab('calendar')} className={`flex items-center space-x-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'calendar' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200'}`}>
              <Calendar size={16} /> <span className="hidden sm:inline">Emploi du Temps</span>
            </button>
      ...
      <main className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="mb-6 flex flex-wrap gap-3 items-center justify-between p-4 bg-slate-800 rounded-xl border border-slate-700 shadow-md">
          ...
          <div className="flex space-x-2">
            {notifPermission !== 'granted' && (
              <button onClick={requestNotificationPermission} className="bg-indigo-600 hover:bg-indigo-500 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors">
                Autoriser
              </button>
            )}
        ...
        {activeTab === 'home' && (
          <div className="space-y-8">
            <div className="bg-gradient-to-r from-indigo-900/40 to-slate-800 rounded-2xl p-6 border border-indigo-500/20 shadow-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-slate-800 rounded-xl p-5 border border-slate-700 shadow-md">
                <h2 className="text-xl font-bold flex items-center gap-2 text-indigo-400 mb-4"><BookOpen size={20}/> Cours du jour ({todayCourses.length})</h2>
              ...
              {/* To-Do */}
              <div className="bg-slate-800 rounded-xl p-5 border border-slate-700 shadow-md">
                <h2 className="text-xl font-bold flex items-center gap-2 text-emerald-400 mb-4"><CheckSquare size={20}/> À faire aujourd'hui ({todayTodos.length})</h2>
                {todayTodos.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {todayTodos.map(todo => (
                      <div key={todo.id} className="flex items-center justify-between bg-slate-900/60 p-3.5 rounded-lg border border-slate-700/40">
                        <div className="flex items-center space-x-3">
                          <button onClick={() => toggleTodo(todo.id)} className="w-5 h-5 rounded-md border border-slate-600 flex items-center justify-center bg-slate-900 hover:border-emerald-500">
          ...
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-center bg-slate-800 p-4 rounded-xl border border-slate-700 gap-4 shadow-lg">
              ...
              <div className="flex space-x-2 bg-slate-900 p-1 rounded-lg border border-slate-700/60">
                <button onClick={handlePrevWeek} className="p-2 text-slate-400 hover:text-white rounded-md"><ChevronLeft size={18} /></button>
                <button onClick={handleGoToToday} className="px-3 py-1 text-xs font-semibold text-slate-200 bg-slate-800 rounded-md">Aujourd'hui</button>
                <button onClick={handleNextWeek} className="p-2 text-slate-400 hover:text-white rounded-md"><ChevronRight size={18} /></button>
              </div>
            </div>
            ...
            <form onSubmit={handleAddCourse} className="bg-slate-800 rounded-xl p-5 border border-slate-700 shadow-md space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 items-end pt-2 border-t border-slate-700/50">
                <div>
                  <select value={newCourse.repeat} onChange={e => setNewCourse({...newCourse, repeat: e.target.value})} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white header-select">
                ...
                <div>
                  <div className="flex items-center space-x-2 bg-slate-900 p-1.5 rounded-lg border border-slate-700">
                    {colors.map(c => (
                      <button type="button" key={c.id} onClick={() => setNewCourse({...newCourse, color: c.id})} className={`w-6 h-6 rounded-full ${c.bg} ${newCourse.color === c.id ? 'ring-2 ring-white scale-110' : 'opacity-60'}`} />
                    ))}
                  </div>
                </div>
                <div className="md:col-span-1 lg:col-span-2 flex gap-3">
                  <input type="text" placeholder="Salle (Optionnel)" value={newCourse.room} onChange={e => setNewCourse({...newCourse, room: e.target.value})} className="flex-1 bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none" />
                  <button type="submit" className="bg-indigo-600 hover:bg-indigo-500 text-white font-medium px-5 rounded-lg text-sm flex items-center gap-2 h-[38px] shrink-0 transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7 gap-4">
              {weekDays.map((dateObj, idx) => {
                ...
                return (
                  <div key={dayName} className={`bg-slate-800 rounded-xl border p-4 flex flex-col min-h-[340px] ${isCurrentToday ? 'border-indigo-500 ring-1 ring-indigo-500/30' : 'border-slate-700'}`}>
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            <form onSubmit={handleAddTodo} className="bg-slate-800 rounded-xl p-4 border border-slate-700 shadow-md flex flex-col sm:flex-row gap-3">
              ...
              <div className="flex gap-2">
                <input type="date" value={newTodo.dueDate} onChange={e => setNewTodo({...newTodo, dueDate: e.target.value})} className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none" />
                <button type="submit" className="bg-emerald-600 hover:bg-emerald-500 px-4 rounded-lg text-white text-sm font-medium flex items-center gap-1 shrink-0">
